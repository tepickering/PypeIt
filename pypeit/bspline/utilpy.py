...
def bspline_model(x, action, lower, upper, coeff, n, nord, npoly):
    ...
    # np.savez_compressed('bspline_model.npz', x=x, action=action, lower=lower,
    #                     upper=upper, coeff=coeff, n=n, nord=nord, npoly=npoly)
    ...
    for i in range(n - nord + 1):
        ...
        yfit[lower[i] : upper[i] + 1] = np.dot(
            action[lower[i] : upper[i] + 1, :], coeff.flatten("F")[i * npoly + spot]
        )
    return yfit
...
def solution_arrays(nn, npoly, nord, ydata, action, ivar, upper, lower):
    ...
    for k in range(nn - nord + 1):
        if nowidth[k]:
            continue
        itop = k * npoly
        alpha.T.flat[bo + itop * bw] += np.dot(
            a2[lower[k] : upper[k], :].T, a2[lower[k] : upper[k], :]
        ).flat[bi]
        beta[itop : min(itop, nfull) + bw] += np.dot(
            ydata[lower[k] : upper[k]] * np.sqrt(ivar[lower[k] : upper[k]]),
            a2[lower[k] : upper[k], :],
        )
    upper -= 1
    return alpha, beta
