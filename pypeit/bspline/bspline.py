...
class bspline(datamodel.DataContainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # JFH added this to enforce immutability of these input arguments, as this code
        # modifies bkpt and fullbkpt as it goes
        fullbkpt1 = copy.copy(fullbkpt)
        bkpt1 = copy.copy(bkpt)
        if from_dict is not None:
            self.nord = from_dict["nord"]
            self.npoly = from_dict["npoly"]
            self.breakpoints = np.array(from_dict["breakpoints"]).astype(
                float
            )  # Force type
        ...
        # Instantiate empty if neither fullbkpt or x is set
            ...
            if fullbkpt1 is None:
                ...
                """
                JFH added this new code, because bkpt.size = 1 implies fullbkpt has only
                2*(nord-1) + 1 elements. This will cause a crash in action because
                nbkpt < 2*nord, i.e. for bkpt = 1, nord = 4 fullbkpt has seven elements
                which is less than 2*nord = 8. The codes above seem to require nbkpt >=2,
                so I'm implementing this requirement. Note that the previous code before this
                fix simply sets bkpt to bkpt[imax] =x.max() which is equally arbitrary,
                but still results in a crash. By requiring at least 2 bkpt, fullbkpt will
                ...
                # Note that with the JFH change above, this nshortbkpt ==1 is never realized because above I forced
            ...
            self.breakpoints = fullbkpt1.astype(
                float
            )  # Ensure type is float for C extension
            self.nord = nord
            self.npoly = npoly
    ...
    def reinit_coeff(self):
        ...
        self.coeff = (
            np.zeros((self.npoly, nc), dtype=float)
            if self.npoly > 1
    ...
    def fit(self, xdata, ydata, invvar, x2=None):
        ...
        for k in range(nn - self.nord + 1):
            ...
            if ict > 0:
                work = np.dot(
                    a1[lower[k] : upper[k] + 1, :].T, a2[lower[k] : upper[k] + 1, :]
                )
                wb = np.dot(
                    ydata[lower[k] : upper[k] + 1], a2[lower[k] : upper[k] + 1, :]
                )
        ...
        if self.coeff.ndim == 2:
            ...
            self.icoeff[:, goodbk] = np.array(
                a[0, 0:nfull].T.reshape(self.npoly, nn, order="F"), dtype=a.dtype
            )
            self.coeff[:, goodbk] = np.array(
                sol[0:nfull].T.reshape(self.npoly, nn, order="F"), dtype=sol.dtype
            )
        else:
    ...
    def action(self, x, x2=None):
        ...
        if nbkpt < 2 * self.nord:
            warnings.warn(
                "Order ({0}) too low for {1} breakpoints.".format(self.nord, nbkpt)
            )
            return -2, 0, 0
        nx = x.size
        n = nbkpt - self.nord
        ...
        if self.funcname == "poly":
            ...
            # JFH fixed bug here where temppoly needed to be transposed because of
            # different IDL and python array conventions
    ...
    def value(self, x, x2=None, action=None, lower=None, upper=None):
        ...
        if action is None:
            action, lower, upper = self.action(
                x[xsort], x2=None if x2 is None else x2[xsort]
            )
        else:
        ...
        yfit = bspline_model(
            x, action, lower, upper, goodcoeff, n, self.nord, self.npoly
        )
    ...
    def maskpoints(self, err):
        ...
        if np.any(hmm >= n):
            warnings.warn(
                "Note enough unique points in cholesky_band decomposition of b-spline matrix. Returning..."
            )
            return -2
        test = np.zeros(nbkpt, dtype=bool)
        for jj in range(-int(np.ceil(self.nord / 2)), int(self.nord / 2.0)):
            foo = np.where(
                (hmm + jj) > 0, hmm + jj, np.zeros(hmm.shape, dtype=hmm.dtype)
            )
            inside = np.where(
                (foo + self.nord) < n - 1,
                foo + self.nord,
    ...
    def workit(self, xdata, ydata, invvar, action, lower, upper):
        ...
        only solves the linear system once, and stores the coefficients in sset. A non-zero
        return value signifies a failed inversion
        ...
        lower  : `numpy.ndarray`_
            A list of pixel positions, each corresponding to the first occurence of position
            greater than breakpoint indx
        ...
        alpha, beta = solution_arrays(
            nn, self.npoly, self.nord, ydata, action, invvar, upper, lower
        )
        nfull = nn * self.npoly
        ...
        if self.coeff.ndim == 2:
            self.icoeff[:, goodbk] = np.array(
                a[0, :nfull].T.reshape(self.npoly, nn, order="F"), dtype=a.dtype
            )
            self.coeff[:, goodbk] = np.array(
                sol[:nfull].T.reshape(self.npoly, nn, order="F"), dtype=sol.dtype
            )
        else:
...
# TODO: I don't think we need to make this reproducible with the IDL version anymore,
# and can opt for speed instead.
